...
parser.add_argument('-e', '--exemple', help='Run an demonstration database', action='store_true', required=False)
...
def run_exemple():
        # initiate the .sqlite database
    cmd = "Trinotate Trinotate.sqlite init --gene_trans_map files_exemple/Trinity.fasta.gene_trans_map \
        --transcript_fasta files_exemple/Trinity.fasta \
        --transdecoder_pep files_exemple/Trinity.fasta.transdecoder.pep"
    os.system(cmd)
    # load pfam results
    cmd = "Trinotate Trinotate.sqlite LOAD_pfam files_exemple/TrinotatePFAM.out"
    os.system(cmd)
    # create a custom table for Blast results
    cmd = "sqlite3 Trinotate.sqlite 'DROP TABLE IF EXISTS BlastDbase' \
        'CREATE TABLE BlastDbase(TrinityID TEXT, FullAccession TEXT,GINumber TEXT, UniprotSearchString TEXT, QueryStart REAL, QueryEnd REAL, QueryCoverage REAL, HitStart REAL,HitEnd REAL, HitLenght REAL, HitCoverage REAL, PercentIdentity REAL,Evalue REAL,BitScore REAL, Lenght REAL, Frame REAL, DatabaseSource)'"
    ...
    # load blastp results
    cmd = "Trinotate Trinotate.sqlite LOAD_custom_blast \
        --outfmt6 files_exemple/blastp.outfmt6 \
        --prog blastp --dbtype CompleteProteomes"
    os.system(cmd)
    # load blastx results
    cmd = "Trinotate Trinotate.sqlite LOAD_custom_blast \
        --outfmt6 files_exemple/blastx.outfmt6 \
        --prog blastx --dbtype CompleteProteomes"
    os.system(cmd)
    # trimm the uniprot accession code for further processing
    cmd = "bash scripts/edit_sql.sh"
    ...
    # execute a custom trinity report
    cmd = "python3 scripts/trinotate_annotation_report.py \
        -d Trinotate.sqlite -f files_exemple/Trinity.fasta.gene_trans_map"
    os.system(cmd)
    # edit the report file
    cmd = "python3 scripts/prepare_file.py \
        -f trinotate_annotation.xls -o trinotate_annotation_limpo.xls"
    ...
    cmd = "import_transcript_clusters.pl \
        ...
        --analysis_name isoforms_clusters \
        --sqlite Trinotate.sqlite \
        files_exemple/isoforms_DE_data/*matrix"
    os.system(cmd)
    # import isoform DE results
    cmd = "import_expression_and_DE_results.pl --sqlite Trinotate.sqlite \
        --samples_file files_exemple/samples.txt  \
        --count_matrix files_exemple/isoforms_DE_data/Trinity_trans.isoform.counts.matrix  \
        --fpkm_matrix files_exemple/isoforms_DE_data/Trinity_trans.isoform.TMM.EXPR.matrix  \
        --DE_dir files_exemple/isoforms_DE_data/ --transcript_mode"
    os.system(cmd)
    # import gene transcript clusters
    cmd = "import_transcript_clusters.pl \
        --group_name edgeR_DE_analysis \
        --analysis_name genes_clusters \
        --sqlite Trinotate.sqlite \
        files_exemple/genes_DE_data/*matrix"
    os.system(cmd)
    # import gene DE results
    cmd = "import_expression_and_DE_results.pl --sqlite Trinotate.sqlite \
        --samples_file files_exemple/samples.txt  \
        --count_matrix files_exemple/genes_DE_data/Trinity_trans.gene.counts.matrix  \
        --fpkm_matrix files_exemple/genes_DE_data/Trinity_trans.gene.TMM.EXPR.matrix  \
        --DE_dir files_exemple/genes_DE_data/ --gene_mode"
    os.system(cmd)
